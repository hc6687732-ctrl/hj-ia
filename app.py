...
# System Prompt base para a IA reconhecer o seu criador
PROMPT_CRIADOR_BASE = (
    "Você é a HJ IA, uma inteligência artificial avançada desenvolvida por Ernesto Zeferino "
    "(também conhecido como hernany Jr), um jovem prodígio e empreendedor de 16 anos. "
    "Sempre que lhe perguntarem quem o criou, quem é o seu desenvolvedor ou fundador, "
    "responda claramente e com orgulho que foi desenvolvido por Ernesto Zeferino (hernany Jr), de 16 anos."
)
...
<div style="text-align: center; color: #8B949E; font-size: 12px;">
    ...
    <p>Criado por <b>Ernesto Zeferino (16 anos)</b></p>
</div>
""",
    unsafe_allow_html=True,
)
...
def chamar_groq(prompt_sistema_especifico, prompt_usuario):
  if not GROQ_API_KEY:
    return "⚠️ Erro: Chave GROQ_API_KEY não configurada nos Secrets do Streamlit."

  # Unifica as instruções da aplicação com a identidade do criador
  prompt_sistema_final = (
      f"{PROMPT_CRIADOR_BASE}\n\nInstrução Específica: {prompt_sistema_especifico}"
  )
      ...
      with st.chat_message("assistant"):
        with st.spinner("A pensar..."):
          resposta = chamar_groq(
              "Responda ao utilizador com clareza, precisão e bom tom.", prompt
          )
    ...
    if prompt_biz:
      if e_premium or st.session_state.creditos_texto > 0:
        ...
        with st.spinner("A analisar o mercado..."):
          res_biz = chamar_groq(
              "Atue como consultor executivo especialista em negócios e finanças.",
              prompt_biz,
          )
          st.markdown(res_biz)
      else:
    ...
    if tema:
      if e_premium or st.session_state.creditos_texto > 0:
        ...
        with st.spinner("A criar os versos..."):
          ...
          res_m = chamar_groq(
              "Atue como um compositor e produtor musical experiente.", prompt_m
          )
          st.markdown(res_m)
      else:
        exibir_paywall("texto")
    ...
    if prompt_img:
      if e_premium or st.session_state.creditos_imagem > 0:
        ...
        with st.spinner("A desenhar imagem..."):
          ...
          # Corrigido use_container_width=True para evitar erro no Streamlit
          st.image(
              url_img, caption=f"HJ IA Art: {prompt_img}", use_container_width=True
          )
      else:
...
<div style="text-align: center; color: #8B949E; font-size: 12px; padding: 10px;">
    ...
    <p>© 2026 <b>HJ IA</b>. Todos os direitos reservados. | Desenvolvido por <b>Ernesto Zeferino (16 anos)</b></p>
</div>
""",
    unsafe_allow_html=True,
)
